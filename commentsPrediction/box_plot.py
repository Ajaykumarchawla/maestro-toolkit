...
def draw_box_plot(data, title="Box Plot of Description Sizes"):
    ...
    q2 = np.percentile(filtered_data, 50)
    ...
    # plt.boxplot(filtered_data, showfliers=False,showmeans=True)  # exclude outliers
    # plt.title(title)
    ...
    plt.boxplot(
        ...
        showmeans=False,    # Include mean value
        meanline=False,     # Show mean as a line
        patch_artist=True  # Use color-filled boxes
    )

    # Add labels and title
    # plt.title("Boxplot of Description Sizes", fontsize=14, fontweight='bold')
    plt.ylabel("Size of Description", fontsize=12)
    # plt.xlabel("Box plot", fontsize=12)

    # Enhance grid and visual presentation
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.tight_layout()

    # Save the plot to a file (optional, for thesis inclusion)
    plt.savefig("description_boxplot.pdf", dpi=300, bbox_inches='tight')
